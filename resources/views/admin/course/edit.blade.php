<x-app-layout>
    <div class="py-12 px-10 text-gray-700">
        ...
        <p><a href="{{ route('admin.course') }}">Course</a> / <a href="{{ route('admin.course.detail', $courseCategory->id)  }}">
            {{ $courseCategory->name }}</a> / <a href="{{ route('admin.course.detailCourse', $course->id)  }}">{{ $course->courseName }}</a> / Edit</p>

        <form action="{{ route('admin.course.update', $course->id) }}" method="post" enctype="multipart/form-data" class="rounded-lg bg-[#133256] p-5 mt-5">
            @csrf
            @method('patch')
            ...
            <div>
                ...
                <x-text-input id="courseName" class="block mt-1 w-full" type="text" name="courseName" value="{{ $course->courseName }}" required autofocus />
            ...
            <div class="mt-4">
                ...
                <x-text-input id="namaPemateri" class="block mt-1 w-full" type="text" name="namaPemateri" value="{{ $course->namaPemateri }}" required />
            ...
            <div class="mt-4">
                ...
                <textarea name="deskripsi" class="textarea textarea-bordered block mt-1 w-full bg-white" placeholder="Deskripsi Course" required>{{ $course->deskripsi }}</textarea>
                <x-input-error :messages="$errors->get('deskripsi')" class="mt-2" />
            </div>
            <div class="mt-4">
                <x-input-label for="file" :value="__('Photo')" class="text-white"/>
                <input type="file" name="file" class="file-input file-input-bordered block mt-1 w-full bg-white"/>
                <x-input-error :messages="$errors->get('file')" class="mt-2" />
            </div>
            <div class="mt-4">
                <x-input-label for="sertifikat" :value="__('Sertifikat')" class="text-white"/>
                <input type="file" name="sertifikat" class="file-input file-input-bordered block mt-1 w-full bg-white"/>
