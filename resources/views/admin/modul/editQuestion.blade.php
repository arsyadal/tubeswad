<x-app-layout>
    <div class="py-12 px-10 text-gray-700">
        <h1 class="text-xl font-bold">Edit Modul Activities</h1>
        ...
        <form action="{{ route('admin.modul.question.update', $activities->id) }}" method="post" enctype="multipart/form-data"
            ...
            <div>
                ...
                <select name="modulType" id="select"
                    ...
                    <option value="Presentation Materi" {{ $activities->modulType == 'Presentation Materi'? 'selected' : ""}}>Presentation Materi</option>
                    <option value="Materi PDF" {{ $activities->modulType == 'Materi PDF'? 'selected' : ""}}>Materi PDF</option>
                    <option value="Post-Test" {{ $activities->modulType == 'Post-Test'? 'selected' : ""}}>Post-Test</option>
                    <option value="Video" {{ $activities->modulType == 'Video'? 'selected' : ""}}>Video</option>
                </select>
            ...
            <div class="mt-4" id="deskripsi">
                ...
                name="deskripsi" :value="old('deskripsi')" required>{{ $activities->deskripsi }}</textarea>
                <x-input-error :messages="$errors->get('materi')" class="mt-2" />
            </div>
